    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('confirmDelete').addEventListener('click', function() {
                if (deleteArticleId) {
                    // Perform AJAX request untuk menghapus artikel
                    fetch(`/articles/${deleteArticleId}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
                        ...
                        .then(data => {
                            if (data.success) {
                                // Hapus baris artikel yang telah dihapus
                                document.querySelector(`tr[data-id="${deleteArticleId}"]`).remove();
                                showToast('success',
                                    'Article deleted successfully!'
                                ); // Show success toast for updating
                                hideModal(); // Sembunyikan modal setelah delete berhasil
                            }
                        })
                        .catch(error => {
                            console.error('Error:', error);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            statusDropdowns.forEach(dropdown => {
                ...
                dropdown.addEventListener('change', function() {
                    ...
                    const articleId = this.dataset.id;
                    fetch(`/articles/${articleId}/update-status`, {
                            method: 'PATCH',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
